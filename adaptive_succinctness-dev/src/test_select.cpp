...
//typedef RunEncoderSelect<16, 256, 64, sdsl::sd_vector<>, sdsl::select_support_sd<1>, sdsl::sd_vector<>,
//                         sdsl::select_support_sd<1>,
//                         sdsl::rank_support_sd<1>> res_256_sd_64;
//typedef RunEncoderSelect<16, 128, 64, sdsl::sd_vector<>, sdsl::select_support_sd<1>, sdsl::sd_vector<>,
//                         sdsl::select_support_sd<1>,
//                         sdsl::rank_support_sd<1>> res_128_sd_64;
//typedef RunEncoderSelect<16, 64, 64, sdsl::sd_vector<>, sdsl::select_support_sd<1>, sdsl::sd_vector<>,
//                         sdsl::select_support_sd<1>,
//                         sdsl::rank_support_sd<1>> res_64_sd_64;

typedef RunEncoderSelect<16, 256, 64, sdsl::s18::vector<256>, sdsl::s18::select_support<1, 256>, sdsl::sd_vector<>,
                         sdsl::select_support_sd<1>,
                         sdsl::rank_support_sd<1>> res_256_sd_64;
typedef RunEncoderSelect<16, 128, 64, sdsl::s18::vector<256>, sdsl::s18::select_support<1, 256>, sdsl::sd_vector<>,
                         sdsl::select_support_sd<1>,
                         sdsl::rank_support_sd<1>> res_128_sd_64;
typedef RunEncoderSelect<16, 64, 64, sdsl::s18::vector<256>, sdsl::s18::select_support<1, 256>, sdsl::sd_vector<>,
                         sdsl::select_support_sd<1>,
                         sdsl::rank_support_sd<1>> res_64_sd_64;
...
//typedef RunEncoderSelect<16, 128, 64, la_vector_opt<uint64_t>, select_support_la, sdsl::sd_vector<>,
//                         sdsl::select_support_sd<1>,
//                         sdsl::rank_support_sd<1>> res_128_la_64;
//typedef RunEncoderSelect<16, 64, 64, la_vector_opt<uint64_t>, select_support_la, sdsl::sd_vector<>,
//                         sdsl::select_support_sd<1>,
//                         sdsl::rank_support_sd<1>> res_64_la_64;
...
int main() {
  ...
  Randomer r_randomer{0, seq[seq.size() - 1] - 1, 1048};
  Randomer s_randomer{1, seq.size() - 1, 1048}; // 2048
  ...
  {
    sdsl::sd_vector<> sd(seq.begin(), seq.end());
   ...
   // time_span_s = chrono::duration_cast< chrono::microseconds >(stop_s - start_s).count();
    //total_time_s = time_span_s.count();
    total_time_s = chrono::duration_cast< chrono::microseconds >(stop_s - start_s).count();
    ...
    print_info("SD VECTOR", total_size, (double) total_size / seq.size(), (double) total_size / (seq.back() + 1),
               Q, (double) total_time * 1000000 / n_queries, Q_s, (double) total_time_s / n_queries);
  }
  std::vector< uint64_t > ks = {4, 8, 16, 32, 64, 128}; 
  {
    for(uint64_t k : ks) {
...
//      total_time_s = time_span_s.count();
//
//      print_info("SELECT LA", 64, 64, k, run.bits_tunstall_seq(),
//                 (double) run.bits_tunstall_seq() / seq.size(),
//                 (double) run.bits_tunstall_seq() / (seq.back() + 1),
//                 q, total_time * 1000000 / n_queries, q_s, total_time_s * 1000000 / n_queries);
//    }
...
//      total_time_s = time_span_s.count();
//
//      print_info("SELECT LA", 128, 64, k, run.bits_tunstall_seq(),
//                 (double) run.bits_tunstall_seq() / seq.size(),
//                 (double) run.bits_tunstall_seq() / (seq.back() + 1),
//                 q, total_time * 1000000 / n_queries, q_s, total_time_s * 1000000 / n_queries);
//    }
...
//      total_time_s = time_span_s.count();
//
//      print_info("SELECT LA", 256, 64, k, run.bits_tunstall_seq(),
//                 (double) run.bits_tunstall_seq() / seq.size(),
//                 (double) run.bits_tunstall_seq() / (seq.back() + 1),
//                 q, total_time * 1000000 / n_queries, q_s, total_time_s * 1000000 / n_queries);
//    }
