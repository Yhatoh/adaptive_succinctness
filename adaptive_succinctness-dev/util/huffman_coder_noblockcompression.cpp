...
#include <math.h>
#include "huffman_coder_noblockcompression.hpp"
#include "nqsort.c"
#include "mysort.c"
//#include "nqsort.h"
//#include "mysort.h"
...
uint64_t huffman_coder::size_bits()
{
    ...
    uint64_t size_block_info = block_info.size()*sizeof(blockElement); //size_in_bytes(block_info_starting_position) + size_in_bytes(block_info_prefix_sum);
...
{    
    ...
    //int_vector<> block_pos(seq.size()/block_size + 1), block_sum(seq.size()/block_size + 1);
     
    uint64_t last_block = 0;
    blockElement aux_block;
    ...
    for (i = 0; i < seq.size(); ++i) {
        if (i%block_size == 0) {
            ...
            aux_block.starting_position = compressed_seq.size();
            aux_block.prefix_sum = sum;
            block_info.push_back(aux_block);
        }
        sum += (seq[i] - 1);  // -1 because of the +1 done to the original sequence
        total_len += output(seq[i], syms, freq_table);
    }
    
    //block_info_starting_position = enc_vector<>(block_pos);
    //block_info_prefix_sum = enc_vector<>(block_sum);    
...
uint32_t huffman_coder::decode(uint64_t i)
{
    ...
    uint64_t sum = block_info[i/block_size].prefix_sum; //block_info_prefix_sum[i/block_size];
    cur_int = block_info[i/block_size].starting_position; //block_info_starting_position[i/block_size];
